...
def create_network(cfg):
    ...
    elif cfg.MODEL.TYPE == 'ds_unet_mmtm':
        return DualStreamUNetMMTM(cfg)
    else:
        return UNet(cfg)
...
class DualStreamUNetMMTM(nn.Module):

    def __init__(self, cfg):
        super(DualStreamUNetMMTM, self).__init__()
        self._cfg = cfg
        out = cfg.MODEL.OUT_CHANNELS
        topology = cfg.MODEL.TOPOLOGY
        ...
        self.mmtm_encoder = MMTMEncoder(cfg)
        self.mmtm_decoder = MMTMDecoder(cfg)
        self.mmtm_mode = 'fusion'
    ...
    def forward(self, x_sar: torch.Tensor, x_optical: torch.Tensor):
        ...
        # encoding and decoding with mmtm
        features_sar, features_optical = self.mmtm_encoder(x_sar, x_optical)
        x_sar, x_optical = self.mmtm_decoder(features_sar, features_optical)
    ...
    def compute_unimodal_features(self, x_sar: torch.Tensor, x_optical: torch.Tensor):
        ...
        features_sar, features_optical = self.mmtm_encoder(x_sar, x_optical, compute_unimodal_mmtm_features=True)
        self.mmtm_decoder(features_sar, features_optical, compute_unimodal_mmtm_features=True)

    def reset_mmtm_features(self):
        for mmtm in self.mmtm_encoder.mmtm_seq.values():
            mmtm.reset_average_squeeze()
        for mmtm in self.mmtm_decoder.mmtm_seq.values():
            mmtm.reset_average_squeeze()

    def set_mmtm_mode(self, new_mode: str):
        self.mmtm_mode = new_mode
        for mmtm in self.mmtm_encoder.mmtm_seq.values():
            mmtm.set_mode(new_mode)
        for mmtm in self.mmtm_decoder.mmtm_seq.values():
            mmtm.set_mode(new_mode)
...
class MMTMEncoder(nn.Module):
    ...
    def forward(self, x_sar: torch.Tensor, x_optical: torch.Tensor,
                compute_unimodal_mmtm_features: bool = False) -> tuple:
...
class MMTMDecoder(nn.Module):
    def __init__(self, cfg: experiment_manager.CfgNode):
        super(MMTMDecoder, self).__init__()
        self.cfg = cfg
        ...
        up_dict_sar, up_dict_optical, mmtm_dict = OrderedDict(), OrderedDict(), OrderedDict()
        ...
        for idx in reversed(range(n_layers)):
            ...
            up_sar = Up(in_dim * 2, out_dim, DoubleConv)
            up_dict_sar[f'up{idx + 1}_sar'] = up_sar
            up_optical = Up(in_dim * 2, out_dim, DoubleConv)
            up_dict_optical[f'up{idx + 1}_optical'] = up_optical

        self.up_seq_sar = nn.ModuleDict(up_dict_sar)
        self.up_seq_optical = nn.ModuleDict(up_dict_optical)
        self.mmtm_seq = nn.ModuleDict(mmtm_dict)

    def forward(self, features_sar: list, features_optical: list,
                compute_unimodal_mmtm_features: bool = False) -> tuple:

        x1_sar, x1_optical = features_sar.pop(0), features_optical.pop(0)
        for idx, (mmtm, up_sar, up_optical) in enumerate(zip(self.mmtm_seq.values(), self.up_seq_sar.values(),
                                                             self.up_seq_optical.values())):

            if compute_unimodal_mmtm_features:
                mmtm.compute_unimodal_average_squeeze(x1_sar, x1_optical)

            x1_sar_mmtm, x1_optical_mmtm = mmtm(x1_sar, x1_optical)

            x2_sar = features_sar[idx]
            x2_optical = features_optical[idx]

            x1_sar = up_sar(x1_sar_mmtm, x2_sar)

            x1_optical = up_optical(x1_optical_mmtm, x2_optical)

        return x1_sar, x1_optical
